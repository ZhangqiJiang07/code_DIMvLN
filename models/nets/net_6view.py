...
class Net(nn.Module):
	def __init__(self, config, method, classes=2, device=None):
		...
		if method['Completion'] == 'complete_with_gcn':
			...
			# Build sub-GCNs
			self.gcn1 = GCN(config['GCN']['arch1'], config['GCN']['activation1'])
			self.gcn2 = GCN(config['GCN']['arch2'], config['GCN']['activation2'])
			self.gcn3 = GCN(config['GCN']['arch3'], config['GCN']['activation3'])
			self.gcn4 = GCN(config['GCN']['arch4'], config['GCN']['activation4'])
			self.gcn5 = GCN(config['GCN']['arch5'], config['GCN']['activation5'])
			self.gcn6 = GCN(config['GCN']['arch6'], config['GCN']['activation6'])

			# Put into a dictionary
			self.gcn_dic = {}
			self.gcn_dic['gcn1'] = self.gcn1
			self.gcn_dic['gcn2'] = self.gcn2
			self.gcn_dic['gcn3'] = self.gcn3
			self.gcn_dic['gcn4'] = self.gcn4
			self.gcn_dic['gcn5'] = self.gcn5
			self.gcn_dic['gcn6'] = self.gcn6
		...
		if method['Representation'] == 'autoencoder':
			...
			self.ae3 = AutoEncoder(self.config['Autoencoder'][f'arch3'],
									self.config['Autoencoder'][f'activation3'],
									self.config['Autoencoder']['batchnorm'])

			self.ae4 = AutoEncoder(self.config['Autoencoder'][f'arch4'],
									self.config['Autoencoder'][f'activation4'],
									self.config['Autoencoder']['batchnorm'])

			self.ae5 = AutoEncoder(self.config['Autoencoder'][f'arch5'],
									self.config['Autoencoder'][f'activation5'],
									self.config['Autoencoder']['batchnorm'])

			self.ae6 = AutoEncoder(self.config['Autoencoder'][f'arch6'],
									self.config['Autoencoder'][f'activation6'],
									self.config['Autoencoder']['batchnorm'])
			...
			self.ae_dic['ae3'] = self.ae3
			self.ae_dic['ae4'] = self.ae4
			self.ae_dic['ae5'] = self.ae5
			self.ae_dic['ae6'] = self.ae6
	...
	def _eval(self, X, incomplete_input_subadj=None):
	...
	def forward(self, X, mode='train', incomplete_input_subadj=None):
		...
		if mode == 'train':
			...
			return self._eval(X, incomplete_input_subadj)
